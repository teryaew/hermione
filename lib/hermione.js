...
const passthroughEventAsync = require('qemitter/utils').passthroughEvent;
const pluginsLoader = require('plugins-loader');

const passthroughEvent = require('./utils').passthroughEvent;
const RunnerEvents = require('./constants/runner-events');
const AsyncRunnerEvents = require('./constants/async-runner-events');
...
module.exports = class Hermione extends QEmitter {
    ...
    run(testPaths, options) {
        ...
        passthroughEventAsync(runner, this, _getAsyncEvents());
        passthroughEventAsync(signalHandler, this, RunnerEvents.EXIT);

        passthroughEvent(runner, this, _getSyncEvents());
...
function _getAsyncEvents() {
    return _.values(AsyncRunnerEvents);
}

function _getSyncEvents() {
    return _.difference(_.values(RunnerEvents), _.values(AsyncRunnerEvents));
}
