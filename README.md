...
`AFTER_FILE_READ`         | Will be triggered on test files parsing righ after reading the file. The handler accepts data object with `file`, `browser` (browser id string) and `hermione` (helper which will be available in test file) fields.
